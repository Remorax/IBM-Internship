...
do
	for max_path in "${max_paths[@]}";
	do
		jbsub -q x86_24h -mem 40g -require k80 -cores 1x1+1 -out "Results/Output_att"$max_path"_"$max_pathlen"_anatomy_aml.txt" ~/anaconda2/envs/myenv/bin/python3.6 Attention_anatomy_aml.py Input/data_anatomy_oaei.pkl $max_path $max_pathlen "Output/test_anatomy_aml"$max_path"_"$max_pathlen".pkl" "Models/anatomy_aml"$max_path"_"$max_pathlen".pt"
