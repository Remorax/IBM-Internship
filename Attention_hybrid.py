...
class SiameseNetwork(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for i in range(2):
            x, y = self.name_embedding(inputs[i]), self.name_embedding(inputs[1-i])
            
            node_x = x.permute(1,0,2)[:1].permute(1,0,2) # 3993 * 1 * 512
            neighbours_x = x.permute(1,0,2)[1:].permute(1,0,2) # 3993 * 9 * 512
            neighbours_y = y.permute(1,0,2)[1:].permute(1,0,2) # 3993 * 9 * 512            
            neighbours_y_sum = torch.sum(neighbours_y, dim=1).unsqueeze(1)

            att_weights = torch.bmm(neighbours_y_sum, neighbours_x.permute(0, 2, 1)).squeeze()
            att_weights = masked_softmax(att_weights).unsqueeze(-1)
            # context = torch.mean(att_weights * neighbours_x, dim=1)
            context = torch.matmul(self.v, att_weights * neighbours_x)

            x = torch.cat((node_x.reshape(-1, self.embedding_dim), context.reshape(-1, self.embedding_dim)), dim=1)
            x = self.output(x)
            results.append(x)
