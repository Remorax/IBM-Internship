...
do
	...
	do
		for threshold in "${thresholds[@]}";
		do
			jbsub -q x86_24h -mem 40g -require k80 -cores 1x1+1 -out "Results/Output_att"$max_path"_"$max_pathlen"_"$threshold"_conf_oaei_german_logmap.txt" ~/anaconda2/envs/myenv/bin/python3.6 Attention_german_amlconf_oaei.py Input/data_conf_oaei_german_logmap_thresh.pkl $max_path $max_pathlen $threshold  "Output/test_conf_oaei_german_logmap_"$max_path"_"$max_pathlen"_"$threshold".pkl" "Models/conf_oaei_german_logmap_"$max_path"_"$max_pathlen"_"$threshold".pt"
		done
	done
done
