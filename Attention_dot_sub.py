...
class SiameseNetwork(nn.Module):
    def __init__(self):
        ...
        self.output = nn.Linear(self.embedding_dim * 3, 600)
        n = int(sys.argv[1])
        self.v = nn.Parameter(torch.DoubleTensor([1/(n-1) for i in range(n-1)]))
        self.v_sub = nn.Parameter(torch.DoubleTensor([1/(n-1) for i in range(n-1)]))
        self.W_sub = nn.Linear(self.embedding_dim, 1)
